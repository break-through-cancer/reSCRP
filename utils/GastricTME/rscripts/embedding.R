...
g_tissuetype <- ggplot(df) +
    geom_point(mapping = aes(x = UMAP1, y = UMAP2, color = SampleType),
               size = 0.1) +
    scale_color_manual(values = colorRampPalette(umapColor)(length(unique(df$SampleType)))) +
    xlab("UMAP_1") + ylab("UMAP_2") +
    coord_fixed() +
    theme_classic() +
    theme(legend.key.size = unit(0.5, "cm")) +
...
gL <- list(CellType = g_celltype, SampleType = g_tissuetype)
gS <- list(CellType = "", SampleType = "")
