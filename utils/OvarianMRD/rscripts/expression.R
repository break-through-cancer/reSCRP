  ...
  $("#dataset-select").on("change", function () {
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        // Expect result[0] = SampleID list
        //        result[1] = CellType list
        //        result[2] = CellStatus list ✅
        $("#sample-select").html('<option value="">All samples</option>');
        $.each(result[0], function (key, value) {
          const sample = value.SampleID || value.sample_id || value.value || value.label;
          if (sample) {
            $("#sample-select").append(`<option value="${sample}">${sample}</option>`);
          }
        });
        ...
        if (result[2]) {
          $.each(result[2], function (key, value) {
            const status =
              ...
              value.label ||
              "undefined";
            $("#cellstatus-select").append(`<option value="${status}">${status}</option>`);
          });
        }
      },
