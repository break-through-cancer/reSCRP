...
df_ap <- df %>%
    ...
    select(Marker, CellType, avg.exp, pct.exp) %>%
    distinct(Marker, CellType, avg.exp, pct.exp, .keep_all = TRUE) %>%
    ungroup() %>%
    group_by(Marker) %>%
